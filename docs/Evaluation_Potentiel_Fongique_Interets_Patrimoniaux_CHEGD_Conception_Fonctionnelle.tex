...
Afin de faciliter la lecture académique, le document peut être interprété selon
une structuration IMRaD adaptée aux documents de conception :
\begin{itemize}[leftmargin=1.2cm]
  \item \textbf{Introduction} : problématique, objet, contexte et cadre conceptuel.
  \item \textbf{Méthodes} : exigences, règles métier, chaîne de traitement, hypothèses et contrôles.
  \item \textbf{Résultats} : artefacts générés, sorties statistiques et critères d'acceptation.
  \item \textbf{Discussion} : limites, biais, validité, gouvernance et perspectives d'évolution.
\end{itemize}

Cette lecture transversale n'altère pas l'organisation fonctionnelle, mais
améliore l'alignement avec les standards de rédaction scientifique.
...
\url{Evaluation_Potentiel_Fongique_Interets_Patrimoniaux_CHEGD.R}.
...
\subsection{Mise à jour documentaire issue des nouveaux commentaires du code}
La version actuelle du script intègre une \textbf{documentation interne enrichie}
commentant explicitement les objectifs, les entrées/sorties et le rôle de
chaque fonction majeure. La présente conception fonctionnelle est alignée sur
ces commentaires et reprend les axes structurants suivants :
\begin{itemize}[leftmargin=1.2cm]
  \item \textbf{Finalité métier explicitée} : évaluer le potentiel fongique,
  l'intérêt patrimonial et le gradient CHEGD, avec mode optionnel de
  \textit{fidélité Excel}.
  \item \textbf{Traçabilité renforcée} : description détaillée des mécanismes
  de journalisation (initialisation, messages d'information, avertissement, erreur,
  sections, en-tête/résumé/pied de page).
  \item \textbf{Chaîne de traitement clarifiée} : lecture robuste des entrées,
  normalisation, agrégations par site, calculs des indicateurs, restitution
  tabulaire/graphique.
  \item \textbf{Règles de calcul explicitées} : classification du potentiel,
  classification de l'intérêt patrimonial, calcul des gradients par visite et calcul IR.
  \item \textbf{Gestion des cas imparfaits} : conversions défensives,
  résolution robuste de fichier d'entrée, contrôles de colonnes requises et
  messages de diagnostic orientés action.
\end{itemize}
...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item \textbf{Transparence} grâce aux logs, exports intermédiaires et indicateurs d'assurance qualité (QA).
...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item Gestion des droits utilisateurs (IAM) et flux de validation documentaire.
...
Les commentaires de code précisent que cette configuration est conçue pour être
modulable (surcharge possible à terme) tout en conservant un point d'entrée
unique pour la reproductibilité des analyses.
...
Les commentaires de code détaillent explicitement la stratégie de robustesse
de cette étape : détection du séparateur CSV, suppression de colonnes techniques
parasites, conversion sécurisée des dates (formats multiples/numériques Excel)
et conversion sécurisée des numériques (locale à virgule).
...
\subsubsection{\texorpdfstring{Objectif 4 --- Association site $\times$ fiabilité}{Objectif 4 --- Association site x fiabilité}}
\begin{itemize}[leftmargin=1.2cm]
...
\texttt{potentiel\_fongique\_par\_site.csv} & Cibler les sites à fort potentiel biologique. \\
\texttt{indice\_patrimonial\_par\_site.csv} & Argumenter les enjeux de conservation patrimoniale. \\
\texttt{gradient\_chegd\_par\_site.csv} & Suivre la structure CHEGD et sa dynamique entre visites. \\
\texttt{indice\_representativite\_ir\_par\_site.csv} & Évaluer la représentativité relative des observations par site. \\
\texttt{stat\_model\_selection\_summary.csv} & Justifier le choix du modèle le plus pertinent pour la fiabilité. \\
\bottomrule
\end{longtable}
...
Les nouveaux commentaires du code documentent également le rôle opérationnel de
chaque fonction de logging : \texttt{setup\_logging}, \texttt{log\_info},
\texttt{log\_warning\_msg}, \texttt{log\_error}, \texttt{log\_section},
\texttt{log\_header}, \texttt{log\_data\_summary}, \texttt{log\_footer}.
...
Le script produit des indicateurs d'assurance qualité (QA), dont :
\begin{itemize}[leftmargin=1.2cm]
...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item Les résultats fiabilité (Objectifs 1 à 4) sont présents même en cas de mécanisme de repli statistique.
...
\begin{itemize}[leftmargin=1.2cm]
  \item ISO/IEC/IEEE. (2018). \textit{ISO/IEC/IEEE 29148:2018 -- Systems and software engineering -- Life cycle processes -- Requirements engineering}. ISO.
  \item Legendre, P., \& Legendre, L. (2012). \textit{Numerical ecology} (3rd English ed.). Elsevier.
  ...
  \item Suter, G. W. (2006). \textit{Ecological risk assessment} (2nd ed.). CRC Press.
  \item Zuur, A. F., Ieno, E. N., Walker, N. J., Saveliev, A. A., \& Smith, G. M. (2009). \textit{Mixed effects models and extensions in ecology with R}. Springer.
\end{itemize}
