...
Dépôt de publication des scripts R prêts à l\textquotesingle emploi
issus des projets de mycologie et d\textquotesingle écologie.\\
Chaque application est autonome : données d\textquotesingle exemple,
script principal et documentation inclus.
...
\item
  ...
  du potentiel fongique, de l\textquotesingle intérêt patrimonial et du
  gradient CHEGD}
...
\subsection{Interface web locale}\label{interface-web-locale}

Une interface web locale Shiny est disponible pour lancer les scripts
sans modifier leur contenu. Elle fonctionne en local, depuis ce dépôt,
et utilise les répertoires standards \texttt{scripts/}, \texttt{data/},
\texttt{logs/} et \texttt{results/}.

\subsubsection{Lancement}\label{lancement}
...
\tightlist
\item
  \texttt{lancer\_myco\_app.command}
\end{itemize}
...
\tightlist
\item
  \texttt{lancer\_myco\_app\_windows.bat}
\end{itemize}

Les scripts de lancement vérifient la présence de \texttt{Rscript} et
des packages R requis (\texttt{shiny}, \texttt{processx},
\texttt{later}), puis ouvrent l\textquotesingle application locale. Le
script Windows cherche \texttt{Rscript.exe} dans le \texttt{PATH}, puis
dans les emplacements d\textquotesingle installation R habituels
(\texttt{Program\ Files/R/...}).
...
Depuis Windows PowerShell ou l\textquotesingle invite de commandes, le
lancement direct est :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lancer\_myco\_app\_windows.bat}
\end{Highlighting}
\end{Shaded}

Par défaut, l\textquotesingle application écoute sur :
...
\begin{Shaded}
\begin{Highlighting}[]
\VariableTok{MYCO\_APP\_PORT}\OperatorTok{=}\NormalTok{8766 }\ExtensionTok{Rscript}\NormalTok{ app.R}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \texttt{MYCO\_APP\_HOST} : hôte d\textquotesingle écoute, par défaut
  \texttt{127.0.0.1}
\item
  \texttt{MYCO\_APP\_PORT} : port local, par défaut \texttt{8765}
\item
  \texttt{MYCO\_APP\_NO\_BROWSER=true} : ne pas ouvrir automatiquement
  le navigateur
\end{itemize}

\subsubsection{Utilisation}\label{utilisation}

L\textquotesingle application permet de :

\begin{itemize}
\tightlist
\item
  sélectionner un script présent dans \texttt{scripts/} ;
\item
  sélectionner un fichier de données présent dans \texttt{data/} ;
\item
  lancer l\textquotesingle exécution via \texttt{Rscript} ;
\item
  consulter la console d\textquotesingle exécution en direct ;
\item
  afficher le contenu des fichiers logs générés dans \texttt{logs/} ;
\item
  parcourir les résultats par application (\texttt{ICR}, \texttt{CHEGD},
  etc.) ;
\item
  ...
  \tightlist
  \item
    \texttt{CSV} / \texttt{TSV} en tableau ;
  \item
    \texttt{PNG} et images directement dans l\textquotesingle interface
    ;
  \item
    \texttt{PDF} dans un lecteur intégré ;
  \item
    \texttt{TXT} / \texttt{LOG} en texte ;
  \end{itemize}
\item
  fermer le serveur local avec le bouton
  \texttt{Fermer\ l\textquotesingle{}app}.
\end{itemize}

L\textquotesingle interface est organisée en trois onglets :

\begin{itemize}
\tightlist
\item
  \texttt{Console} : flux d\textquotesingle exécution du script lancé ;
\item
  \texttt{Logs} : sélection et affichage du contenu des fichiers logs ;
\item
  \texttt{Résultats} : navigation par application, sélection de fichier
  et aperçu intégré.
\end{itemize}

\subsubsection{Robustesse}\label{robustesse}

La validation des données reste assurée par les scripts R.
L\textquotesingle interface ne permet pas de modifier les scripts. Une
seule exécution est autorisée à la fois afin d\textquotesingle éviter
les collisions entre sorties et fichiers temporaires.

Pour le script CHEGD, qui attend un nom de fichier
d\textquotesingle entrée fixe, l\textquotesingle interface prépare
temporairement le fichier attendu avant l\textquotesingle exécution puis
restaure l\textquotesingle état initial après l\textquotesingle analyse.
Les erreurs de copie, de restauration, de port déjà utilisé ou de
dépendances manquantes sont affichées explicitement.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Évaluation du potentiel fongique, de
l\textquotesingle intérêt patrimonial et du gradient
CHEGD}\label{uxe9valuation-du-potentiel-fongique-de-lintuxe9ruxeat-patrimonial-et-du-gradient-chegd}

Pipeline R dédié à l\textquotesingle évaluation multi-critères des
pelouses sur trois axes :
...
Par défaut, le fichier d\textquotesingle entrée est :
...
\subsubsection{Lancer l\textquotesingle analyse
CHEGD}\label{lancer-lanalyse-chegd}
...
Le journal d\textquotesingle exécution est écrit dans :
...
\textbf{Pour lancer rapidement l\textquotesingle application CHEGD avec
les données du dépôt :}

Cette section suit le même format pas-à-pas que
l\textquotesingle application Inventaires, afin de faciliter la prise en
main des deux pipelines.

\paragraph{Étape 1️⃣ --- Vérifier le fichier
d\textquotesingle entrée}\label{uxe9tape-one--vuxe9rifier-le-fichier-dentruxe9e}
...
script}\label{uxe9tape-two--lancer-le-script}
...
sorties}\label{uxe9tape-three--consulter-les-sorties}
...
avec les tableaux CSV, les figures PNG/PDF et le journal
d\textquotesingle exécution dans \texttt{logs/}.
...
\bottomrule\noalign{}
\endlastfoot
\texttt{Espèces} & Texte & Nom de l\textquotesingle espèce observée \\
\texttt{Famille} & Texte & Famille taxonomique \\
\texttt{Date} & Date ou texte convertible & Date
d\textquotesingle observation \\
...
Méthodes d\textquotesingle exécution identiques à la section Inventaires
: une option en ligne de commande pour la reproductibilité et une option
interactive pour l\textquotesingle exploration.
...
l\textquotesingle exécution}\label{ce-qui-se-passe-pendant-lexuxe9cution}
...
\tightlist
\item
  lit et valide les données d\textquotesingle entrée,
\item
...
active un \textbf{mode d\textquotesingle alignement ``fidélité Excel''}
pour reproduire les valeurs métier du classeur.
...
\bottomrule\noalign{}
\endlastfoot
\texttt{resume\_global.csv} & Vue d\textquotesingle ensemble du jeu
analysé \\
...
\begin{quote}
Le script tente d\textquotesingle installer automatiquement les packages
requis s\textquotesingle ils sont absents.
\end{quote}
...
Exemple d\textquotesingle adaptation :
...
Classement selon l\textquotesingle indice patrimonial :
...
IR\_\{visite\} = \textbackslash max\textbackslash left(0, 1 -
\textbackslash frac\{gradient\_\{visite\}\}\{chegd\_\{total\}\}\textbackslash right)
...
de détermination \\
\texttt{fig7\_indice\_representativite\_ir} & Heatmap de
l\textquotesingle IR par visite et par pelouse \\
\end{longtable}
}
...
\textbf{Le script ne trouve pas mon fichier d\textquotesingle entrée}\\
...
Le mode ``fidélité Excel'' n\textquotesingle est activé que si un
classeur de référence compatible est détecté (feuilles attendues).

\textbf{Je n\textquotesingle ai pas toutes les figures}\\
...
utilisés pour l\textquotesingle évaluation écologique des pelouses. \\
\textbf{IR} & Indice de représentativité calculé par visite : IR =
\textbackslash max(0, 1 - gradient\_\{visite\}/chegd\_\{total\}). \\
\textbf{Potentiel fongique} & Score synthétique basé sur la présence de
groupes et d\textquotesingle espèces indicatrices, puis classé en 3
niveaux. \\
...
l\textquotesingle observation (ex. certaine/probable/non renseignée). \\
\textbf{Mode fidélité Excel} & Mécanisme d\textquotesingle alignement
des sorties script sur un classeur de référence quand les feuilles
...
Pipeline R pour automatiser l\textquotesingle analyse de la
\textbf{complétude d\textquotesingle inventaires fongiques} et de leur
\textbf{représentativité}, via :

\begin{itemize}
\tightlist
\item
  le \textbf{TEE} (Taux d\textquotesingle Espèces Exceptionnelles)
\item
  l\textquotesingle{}\textbf{Ir} (Indice de représentativité : Ir = 1 -
  TEE)
\item
...
open-source. Le script s\textquotesingle exécute avec \textbf{R ≥ 4.0}
sur \textbf{Windows}, \textbf{macOS} et \textbf{Linux}.
...
\item
  \textbf{Sélectionner votre système d\textquotesingle exploitation :}
  ...
  \end{itemize}
\item
  \textbf{Exécuter l\textquotesingle installateur} et suivre les
  instructions par défaut (tout en \textbf{Next} convient)
\item
  \textbf{Vérifier l\textquotesingle installation :} ouvrir un
  \textbf{Terminal} (macOS/Linux) ou \textbf{Invite de Commandes}
  (Windows) et taper :
\end{enumerate}
...
beaucoup plus convivial pour les débutants. Ce n\textquotesingle est pas
obligatoire, mais \textbf{très utile}.
...
\item
  ...
  open-source)
\item
  \textbf{Sélectionner votre système d\textquotesingle exploitation} et
  télécharger l\textquotesingle installateur
\item
  \textbf{Exécuter l\textquotesingle installateur} et suivre les
...
\item
  \textbf{Dans le panneau en bas-à-droite} (onglet "Packages"), cliquer
  sur le bouton \textbf{Install}
\item
  \textbf{Copier-coller} la liste de packages ci-dessous dans la zone
  qui s\textquotesingle ouvre :
\end{enumerate}
...
\tightlist
\item
  \textbf{Vérifier} que "Install from: CRAN repository" est sélectionné
\item
...
\item
  ...
  questions qui s\textquotesingle affichent
\item
  \textbf{Attendre} que l\textquotesingle installation se termine
  (aucune erreur rouge au final ne doit présager un problème ; une
  installation réussie se termine par \texttt{Done})
\end{enumerate}

\subparagraph{Vérifier
l\textquotesingle installation}\label{vuxe9rifier-linstallation}

Après l\textquotesingle installation, exécuter cette commande pour
vérifier que tout les packages requis sont présents :
...
\textbf{Résultat :} vous avez tout ce qu\textquotesingle il faut pour
lancer le script !

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\paragraph{Besoin d\textquotesingle aide ? Dépannage
initial}\label{besoin-daide--duxe9pannage-initial}

\textbf{"R n\textquotesingle est pas reconnu" dans le Terminal/Invite de
Commandes}
...
\item
  ...
  l\textquotesingle installation de R
\item
...
\textbf{"Un package refuse de s\textquotesingle installer"}

\begin{itemize}
\tightlist
\item
  Ignorer pour l\textquotesingle instant --- le script affichera un
  message explicite si un package obligatoire manque
\item
...
\tightlist
\item
  C\textquotesingle est possible, voir section
  \hyperref[ux5cux25EFux5cux25B8ux5cux258F-lancer-le-script--guide-dux5cux25C3ux5cux25A9taillux5cux25C3ux5cux25A9]{Lancer
...
\textbf{Pour les débutants qui viennent d\textquotesingle installer R et
RStudio :}

\paragraph{Étape 1️⃣ --- Préparer vos
données}\label{uxe9tape-one--pruxe9parer-vos-donnuxe9es}

Placer votre fichier d\textquotesingle observations dans le dossier
\texttt{data/} du dépôt, sous le nom \texttt{observations.csv}.

Un fichier d\textquotesingle exemple (\texttt{data/observations.csv})
est \textbf{déjà fourni} --- vous pouvez l\textquotesingle utiliser
directement pour tester !

\paragraph{Étape 2️⃣ --- Ouvrir RStudio et charger le
script}\label{uxe9tape-two--ouvrir-rstudio-et-charger-le-script}
...
\item
  \textbf{Attendre} que l\textquotesingle exécution se termine (vous
  verrez les messages de progression dans la console en bas)
\end{enumerate}

\paragraph{Étape 3️⃣ --- Consulter les
résultats}\label{uxe9tape-three--consulter-les-ruxe9sultats}
...
Si vous êtes à l\textquotesingle aise avec le Terminal, ouvrir un
terminal, naviguer jusqu\textquotesingle à la racine du dépôt et
exécuter :
...
Si vous voyez \texttt{{[}1{]}\ "Hello\ from\ R!\ ✅"},
c\textquotesingle est bon.
...
d\textquotesingle exemple}\label{test-3--lancer-le-script-sur-les-donnuxe9es-dexemple}
...
puis un \textbf{manifeste} en fin d\textquotesingle exécution listant
tous les fichiers créés.

Si tout s\textquotesingle est bien passé, consultez le dossier
\texttt{results/} --- vous y trouverez les fichiers CSV et PNG.
...
Le fichier d\textquotesingle entrée doit être en format \textbf{CSV} (ou
TSV / TXT, détection automatique du séparateur).
...
\texttt{V2}\ldots) \\
\texttt{espece} & Texte & Nom de l\textquotesingle espèce observée \\
\end{longtable}
}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{placette} & Texte & Identifiant de la placette (active
l\textquotesingle analyse spatiale et la CA/AFC) \\
\end{longtable}
}
...
données d\textquotesingle exemple fonctionnel avec plusieurs sites et
placettes.
...
\paragraph{Contrôle de l\textquotesingle exécution via variable
d\textquotesingle environnement}\label{contruxf4le-de-lexuxe9cution-via-variable-denvironnement}
...
l\textquotesingle analyse automatiquement au chargement \\
...
\paragraph{Ce qui s\textquotesingle affiche dans la console pendant
l\textquotesingle exécution}\label{ce-qui-saffiche-dans-la-console-pendant-lexuxe9cution}
...
En fin d\textquotesingle exécution, le \textbf{manifeste} liste
l\textquotesingle état de chaque fichier attendu :
...
\begin{longtable}[]{@{}lll@{}}
\toprule\noalign{}
Option & Défaut & Effet si \texttt{TRUE} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
d\textquotesingle exécution de chaque calcul \\
\texttt{CLEAN\_ENVIRONMENT} & \texttt{FALSE} & Nettoie
l\textquotesingle environnement R avant exécution (usage isolé
uniquement) \\
\end{longtable}
}
...
est créé automatiquement s\textquotesingle il n\textquotesingle existe
pas).

\paragraph{Vue d\textquotesingle ensemble du pipeline (9 étapes →
...
l\textquotesingle indice Ir entre sites \\
...
\texttt{ICR\_02\_tee\_ir.csv} & CSV & Toujours & Évolution de TEE et Ir
au fil des visites \\
\texttt{ICR\_03\_frequence\_especes.csv} & CSV & Toujours & Fréquences
d\textquotesingle occurrence des espèces \\
...
\texttt{ICR\_03\_tee\_ir.png} & Graphique & Toujours & TEE et Ir en deux
panneaux \\
\texttt{ICR\_04\_histogramme\_frequences.png} & Graphique & Toujours &
Distribution des fréquences d\textquotesingle observabilité \\
...
\bottomrule\noalign{}
\endlastfoot
\texttt{minpack.lm} & Ajustement hyperbolique (estimation
d\textquotesingle asymptote, ratio de complétude) \\
...
commande d\textquotesingle installation explicite si certains sont
manquants. L\textquotesingle exécution s\textquotesingle arrête
proprement avec un message clair plutôt que de produire des résultats
partiels silencieux.
\end{quote}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsubsection{⚙️ Configuration}\label{gear-configuration}

La configuration est centralisée dans l\textquotesingle objet
\texttt{CONFIG} au début du script
...
fichier d\textquotesingle entrée (relatif à la racine du projet) \\
\texttt{output\_dir} & \texttt{results/ICR} & Dossier de sortie des CSV
et graphiques \\
\texttt{date\_format} & \texttt{\%Y-\%m-\%d} & Format de date R attendu
(ex. \texttt{2026-03-18}) \\
\texttt{csv\_strict\_mode} & \texttt{TRUE} & Stoppe
l\textquotesingle exécution si le CSV n\textquotesingle est pas
conforme \\
...
\texttt{make\_ca} & \texttt{TRUE} & Active l\textquotesingle ordination
CA/AFC si les conditions sont réunies \\
...
\textbf{Variables d\textquotesingle environnement utiles :}
...
\paragraph{TEE -- Taux d\textquotesingle Espèces
Exceptionnelles}\label{tee--taux-despuxe8ces-exceptionnelles}

Proportion d\textquotesingle espèces observées \textbf{une seule fois}
dans l\textquotesingle inventaire.
...
\tightlist
\item
  \textbf{TEE élevé en fin de suivi} : beaucoup
  d\textquotesingle espèces vues qu\textquotesingle une fois →
  inventaire en phase d\textquotesingle accumulation.
\item
...
\textbackslash text\{Complétude\} =
\textbackslash frac\{S\_\{obs\}\}\{S\_\{asymptote\}\}
...
\textless{} 0,70 & Insuffisante pour conclure sur
l\textquotesingle exhaustivité \\
\end{longtable}
}

\begin{quote}
L\textquotesingle asymptote ne doit pas être interprétée comme une
"vérité absolue", mais comme un \textbf{repère quantitatif} de
complétude.
\end{quote}

\paragraph{Score de pertinence scientifique (agrégé,
v1.4)}\label{score-de-pertinence-scientifique-agruxe9guxe9-v14}

\textbackslash text\{Score\} = 0\{,\}35 \textbackslash times Ir +
0\{,\}35 \textbackslash times \textbackslash text\{complétude\} +
0\{,\}20 \textbackslash times \textbackslash text\{stabilité\} +
0\{,\}10 \textbackslash times R\^{}2

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsubsection{📈 Lecture des
graphiques}\label{chart_with_upwards_trend-lecture-des-graphiques}

\paragraph{\texorpdfstring{\texttt{ICR\_01\_richesse\_par\_visite\_et\_cumul.png}}{ICR\_01\_richesse\_par\_visite\_et\_cumul.png}}\label{icr_01_richesse_par_visite_et_cumulpng}
...
\item
  ...
  observateur.
\item
  Signal d\textquotesingle alerte : si les dernières visites apportent
  encore beaucoup de nouvelles espèces, l\textquotesingle arrêt de
  l\textquotesingle inventaire peut être prématuré.
\end{itemize}

\paragraph{\texorpdfstring{\texttt{ICR\_02\_courbe\_temps\_especes\_hyperbole.png}}{ICR\_02\_courbe\_temps\_especes\_hyperbole.png}}\label{icr_02_courbe_temps_especes_hyperbolepng}
...
\item
  ...
  modèle (saisonnalité forte, ruptures d\textquotesingle effort, etc.).
\item
  \textbf{Asymptote nettement au-dessus de la richesse actuelle} → marge
  de découverte encore importante.
\end{itemize}

\paragraph{\texorpdfstring{\texttt{ICR\_03\_tee\_ir.png}}{ICR\_03\_tee\_ir.png}}\label{icr_03_tee_irpng}

Deux panneaux complémentaires :

panneau haut : nombre d\textquotesingle espèces observées une seule fois
(\texttt{nb\_esp\_1fois})

panneau bas : indice Ir sur l\textquotesingle échelle {[}0,1{]} avec
seuils visuels (0,60 et 0,80)

\textbf{Espèces uniques qui ralentissent} → signal de consolidation de
l\textquotesingle inventaire.
...
\paragraph{\texorpdfstring{\texttt{ICR\_04\_histogramme\_frequences.png}}{ICR\_04\_histogramme\_frequences.png}}\label{icr_04_histogramme_frequencespng}

Distribution du nombre d\textquotesingle espèces selon leur fréquence
d\textquotesingle occurrence.
...
\item
  \textbf{Répartition plus équilibrée} → noyau d\textquotesingle espèces
  régulières bien documenté.
\item
  \textbf{Queue vers les fortes fréquences} → présence
  d\textquotesingle espèces constantes, potentiels bons indicateurs
  écologiques.
\end{itemize}

\paragraph{\texorpdfstring{\texttt{ICR\_05\_temporel\_vs\_spatial.png}
\emph{(si} \texttt{\_placette\_}
\emph{disponible)}}{ICR\_05\_temporel\_vs\_spatial.png (si \_placette\_ disponible)}}\label{icr_05_temporel_vs_spatialpng-si-_placette_-disponible}

Nuage de points reliant fréquence temporelle (x = proportion de visites)
et occupation spatiale (y = proportion de placettes).
...
Ce diagramme permet de distinguer rareté vraie, rareté
d\textquotesingle observation et hétérogénéité spatiale.

\paragraph{\texorpdfstring{\texttt{ICR\_06\_CA\_placettes\_especes.png}
\emph{(si} \texttt{\_vegan\_} \emph{+} \texttt{\_placette\_}
\emph{disponibles)}}{ICR\_06\_CA\_placettes\_especes.png (si \_vegan\_ + \_placette\_ disponibles)}}\label{icr_06_ca_placettes_especespng-si-_vegan_--_placette_-disponibles}
...
\paragraph{\texorpdfstring{\texttt{ICR\_07\_metrics\_pertinence\_dashboard.png}}{ICR\_07\_metrics\_pertinence\_dashboard.png}}\label{icr_07_metrics_pertinence_dashboardpng}
...
\item
  ...
  \textbf{0,80} (bon).
\item
  Tout indicateur sous 0,60 motive un complément
  d\textquotesingle effort ou une révision du protocole.
\end{itemize}
...
\begin{longtable}[]{@{}ll@{}}
\toprule\noalign{}
Graphique & Ce qu\textquotesingle il montre \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\texttt{ICR\_comparaison\_ir\_sites.png} & Sites classés par Ir final
--- prioriser les revisites (sites à Ir faible en premier) \\
...
d\textquotesingle interprétation}\label{bonnes-pratiques-dinterpruxe9tation}
...
\item
  ...
  spatial.
\item
  Toujours contextualiser avec : saison, pression
  d\textquotesingle échantillonnage, météo, observateurs.
\item
  Préférer les \textbf{tendances sur plusieurs visites} plutôt
  qu\textquotesingle une lecture ponctuelle.
\item
  Un site bas en complétude n\textquotesingle est pas "mauvais" : il
  peut être plus hétérogène ou plus difficile à prospecter.
\item
  Croiser complétude et Ir avant de conclure sur
  l\textquotesingle effort à fournir : un Ir élevé avec une complétude
  faible peut signifier un site très riche, pas un protocole défaillant.
\item
  Pour des décisions de gestion, combiner ces indicateurs avec
  l\textquotesingle expertise terrain et la connaissance taxonomique.
\end{itemize}
...
\textbf{Le script ne trouve pas mon fichier d\textquotesingle entrée}\\
Vérifier qu\textquotesingle un fichier existe dans \texttt{data/} sous :
...
\textbf{Le script s\textquotesingle arrête avec "CSV non conforme"}\\
Consulter :
...
\textbf{Le modèle hyperbolique n\textquotesingle est pas calculé}\\
Installer \texttt{minpack.lm}. Sans ce package,
l\textquotesingle analyse continue mais sans asymptote hyperbolique ni
ratio de complétude. Vérifier aussi que le site a au moins
\texttt{CONFIG\$min\_visits\_for\_model} visites (défaut : 5).

\textbf{La CA/AFC n\textquotesingle apparaît pas}\\
...
\item
  ...
  d\textquotesingle exécution pour l\textquotesingle erreur associée.
\item
...
pour identifier les goulots d\textquotesingle étranglement. Sur de
grands jeux de données, la phase de calculs cumulés bénéficie de la
vectorisation introduite en v1.2.
...
données d\textquotesingle entrée, détection automatique du séparateur
(CSV/TSV/TXT), messages d\textquotesingle erreur explicites \\
\end{longtable}
}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{TEE} & Taux d\textquotesingle Espèces Exceptionnelles :
proportion d\textquotesingle espèces observées une seule fois \\
...
de l\textquotesingle inventaire vers la richesse théorique \\
\textbf{Robustesse} & Qualité d\textquotesingle ajustement (R\^{}2) du
modèle d\textquotesingle asymptote \\
...
\textbf{Score pertinence} & Indicateur agrégé pondéré : 0\{,\}35
\textbackslash times Ir + 0\{,\}35 \textbackslash times
\textbackslash text\{complétude\} + 0\{,\}20 \textbackslash times
\textbackslash text\{stabilité\} + 0\{,\}10 \textbackslash times
R\^{}2 \\
...
data Group Analysis}) : outils d\textquotesingle analyse en écologie des
communautés --- ordinations (CA/AFC, PCA, NMDS), indices de diversité,
tests de permutation, etc. Dans ce projet, utilisé pour
l\textquotesingle ordination CA/AFC entre placettes et espèces \\
\end{longtable}
}
